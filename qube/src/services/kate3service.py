...
class kate3Service:
    ...
    def find_by_id(self, entity_id):
        ...
        data = kate3.query.get(entity_id)
        if data is None:
            raise kate3ServiceError(
                'kate3 {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = kate3.query.filter(
            kate3.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = kate3.query.get(entity_id)  # kate3 is a mongo class
        if record is None:
            raise kate3ServiceError(
                'kate3 {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
